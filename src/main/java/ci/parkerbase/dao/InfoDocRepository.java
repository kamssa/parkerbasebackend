...
public interface InfoDocRepository extends JpaRepository<InfoDoc, Long>{
	...
	@Query("select doc from InfoDoc doc where doc.departement.id=?1 ORDER BY doc.nomDoc ASC")
	...
	@Query("select doc from InfoDoc doc where doc.dossier.id=?1 ORDER BY doc.nomDoc ASC")
